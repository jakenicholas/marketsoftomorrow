...
(function () {
  ...
  // stats.json (generated by fetch_projects.py from the real project list) is
  // the one number every surface must show — the /pro hero, paywalls, the dock
  // + account menus, the pulse fallback, the /media pitch. They read it here
  // instead of hardcoding a count that drifts and makes us look like we're
  // lying. Any element with data-tmw-projects gets its text filled; render
  // functions read window.tmwProjectCount() live. The seed below is the true
  // current count so the number is correct even before the fetch resolves.
  window.tmwStats = window.tmwStats || { projects: 965, markets: '40+' };
  window.tmwProjectCount = function(){ return (window.tmwStats && window.tmwStats.projects) || 965; };
  window.tmwProjectCountStr = function(){ return window.tmwProjectCount().toLocaleString('en-US'); };
  window.tmwFillCounts = function(root){
    var scope = root || document;
    var n = window.tmwProjectCountStr();
    try { scope.querySelectorAll('[data-tmw-projects]').forEach(function(el){ el.textContent = n; }); } catch(_){}
  };
  __tmwSharedJson('https://www.oftmw.com/map/stats.json').then(function(s){
    if (s && typeof s.projects === 'number' && s.projects > 0){
      window.tmwStats.projects = s.projects;
      if (s.markets) window.tmwStats.markets = s.markets;
    }
    window.tmwFillCounts();
  });
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', function(){ window.tmwFillCounts(); });
  else window.tmwFillCounts();
  ...
  // Plain static search icon. The spinning-hexagon "Ask TMW" teaser was
  // reverted 2026-08-21; an AI-suggestive icon replacement is being mocked.
  var ICON_SEARCH =
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' +
  ...
  var css = [
    ...
    // Native placeholder stays transparent; the overlay span carries the text
    // (it has responsive large/small variants the native attribute can't do).
    '.tmw-dock-search input::placeholder{color:transparent}',
    '.tmw-dock-search .ds-ph{position:absolute;top:50%;left:42px;right:16px;transform:translateY(-50%);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#9AA39C;font-size:14px;line-height:1;z-index:1;pointer-events:none}',
    '.tmw-dock-search .dph-sm{display:none}',
    // Once the field has text, drop the placeholder overlay (icon stays).
    '.tmw-dock-search.ds-filled .ds-ph{opacity:0}',
  ...
  function mount() {
    ...
    dock.innerHTML =
      ...
      '<form class="tmw-dock-search" role="search" action="' + SEARCH_PAGE + '" method="get">' +
        ...
        // Placeholder overlay (responsive large/small variants). Hidden when the
        // field has text (.ds-filled, toggled in JS).
  ...
  var SEARCH_V = '20260821-namedhero';
  ...
  function theMapPanel() {
    ...
    return '<div class="tmw-mm">' +
      ...
      '<div class="tmw-mm-pro-grid">' +
        ...
        '<a class="tmw-mm-item" href="' + U + '"><span class="tmw-mm-ic">' + ic('<path d="M9 18l-6 3V6l6-3 6 3 6-3v15l-6 3-6-3z"/><path d="M9 3v15M15 6v15"/>') + '</span><span class="tmw-mm-tx"><b>Interactive Map<em>PRO</em></b><i>' + window.tmwProjectCountStr() + ' projects across 40+ markets.</i></span></a>' +
