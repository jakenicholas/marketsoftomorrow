...
async function handleDesignFromPost(req, env, origin) {
  ...
  if (env.ANTHROPIC_API_KEY) {
    const sys = 'You are the social copywriter for Markets of Tomorrow (TMW). Write the Instagram CAROUSEL copy for the article below, obeying TMW\'s carousel rules EXACTLY.\n\n'
      ...
      + '- Slides 2 onward run ~55 to 80 characters (about 2 to 3 lines) — the meaty, information-carrying lines. The text box is NARROW, so anything longer wraps to 5+ lines and crowds the byline; keep each one tight and single-spine.\n'
      + '- Slide 1 is the SHORTEST of all slides (~40 to 60 characters): one punchy, hype, complete-sentence hook, the scroll-stopper.\n'
      ...
      + '- No exact calendar dates or street addresses on SLIDES (soften to a season or "coming soon"); a rough half-year like "early 2027" is fine in the CAPTION. American units and spelling. Positive framing (say what it IS, never what it isn\'t). No em dashes (use a colon or comma). Never invent facts, prices, counts, or names not in the article.\n\n'
      ...
      + 'CAPTION STRUCTURE (three short paragraphs, complete sentences with periods):\n'
      + '- P1 — lead with the project NAME and a tight descriptor, its status or superlative, and the news/reveal (what just happened). Name the project in the first few words.\n'
      + '- P2 — the specs (height, stories, home size/count) AND the design credits (architect/designer, plus notable context like a firm\'s first US commission). One to two sentences; use the real figures from the article.\n'
      + '- P3 — ONE crisp line on timing + access (showings, completion window).\n'
      + 'Then close with exactly: Explore more with the link in our bio 👉\n\n'
      + 'GOLD-STANDARD CAPTION — match this SHAPE and density (not the specifics):\n'
      ...
      + 'At 860 feet and 52 stories, 262 Fifth Avenue is one of the skinniest towers of its height anywhere in New York. The full-floor, column-free homes will average roughly 3,200 square feet, with restrained interiors by Copenhagen\'s Norm Architects and design by Meganom, the firm\'s first American commission. '
      + 'Private showings began this spring, with completion expected in early 2027."\n\n'
      + 'Respond with ONLY JSON:\n'
      + '{"market":"<exactly one of: ' + MARKETS.join(' | ') + '>",'
      + '"location":"<the project CITY for the cover pin, e.g. New York>",'
      + '"caption":"<the IG caption per the structure + gold example above, then the exact bio CTA>",'
...
// Email the weekly brain report card to the team (reuses the worker's Resend setup).
async function sendBrainReportEmail(env, card) {
  ...
  const html = '<div style="background:#EFE7D5;padding:34px 20px"><div style="max-width:560px;margin:0 auto;background:#F7F1E4;border:1px solid #E4D7BE;border-radius:16px;padding:30px 28px;font-family:Inter,Arial,sans-serif">' +
    ...
    '<div style="font-size:10.5px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;color:#B0603A;margin:20px 0 4px">Self-tuning this week</div>' +
    '<table style="border-collapse:collapse">' + optRows + '</table>' +
    '<div style="font-size:10.5px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;color:#B0603A;margin:20px 0 4px">Brain size</div>' +
...
const ADVERTISER_SEED = [
  {id:'south-flagler-house', name:'South Flagler House', type:'partner', active:1, all:1104, d30:6},
  {id:'Humanaut', name:'Humanaut Health Palm Beach Gardens', type:'ad', active:1, all:992, d30:41},
  {id:'pur-form', name:'PURFORM Health', type:'partner', active:1, all:988, d30:9},
  {id:'waldorf-astoria-st-pete', name:'Waldorf Astoria Residences St Petersburg', type:'ad', active:1, all:832, d30:58},
  {id:'spina-o-rourke-partners', name:'Spina O\'Rourke + Partners', type:'partner', active:1, all:587, d30:8},
  {id:'ponce', name:'Ponce Park Coral Gables', type:'ad', active:1, all:545, d30:53},
  {id:'eudemonia', name:'Eudemonia 2026', type:'ad', active:1, all:429, d30:51},
  {id:'higher-order', name:'Higher Order Health', type:'partner', active:1, all:382, d30:16},
  {id:'medhouse', name:'MedHouse', type:'partner', active:1, all:238, d30:11},
  {id:'mandarin-oriental', name:'Mandarin Oriental Residences, West Palm Beach', type:'ad', active:1, all:194, d30:174},
  {id:'tremble', name:'TREMBLE', type:'partner', active:1, all:165, d30:2},
  {id:'traphouse', name:'TRAPHOUSE Palm Beach', type:'partner', active:1, all:20, d30:6},
  {id:'humanaut-health', name:'Humanaut Health', type:'ad', active:1, all:0, d30:0},
  {id:'marina-pointe', name:'Marina Pointe', type:'ad', active:0, all:1943, d30:40},
  {id:'400-central', name:'400 Central', type:'ad', active:0, all:1026, d30:23},
  {id:'related-group', name:'Related Group', type:'ad', active:0, all:692, d30:3},
  {id:'ora-hotel', name:'ORA Hotel', type:'ad', active:0, all:655, d30:2},
  {id:'alba-banner', name:'Alba Banner', type:'ad', active:0, all:573, d30:8},
  {id:'ara-jupiter', name:'Ara Jupiter | Globally-Inspired Menu', type:'ad', active:0, all:555, d30:4},
  {id:'br-z', name:'BRĒZ', type:'ad', active:0, all:468, d30:4},
  {id:'sushi-by-bou', name:'Sushi by Bou', type:'ad', active:0, all:442, d30:3},
  {id:'frida-kahlo-residences', name:'Frida Kahlo Residences', type:'ad', active:0, all:429, d30:1},
  {id:'celis', name:'Celis', type:'ad', active:0, all:423, d30:2},
  {id:'nautilus-220', name:'Nautilus 220', type:'ad', active:0, all:343, d30:2},
  {id:'the-berkeley-palm-beach', name:'The Berkeley Palm Beach', type:'ad', active:0, all:267, d30:2},
  {id:'ziggurat', name:'Ziggurat', type:'ad', active:0, all:248, d30:1},
  {id:'olara-west-palm-beach', name:'Olara West Palm Beach', type:'ad', active:0, all:162, d30:8},
  {id:'havn-residences-yacht-club', name:'HAVN Residences & Yacht Club', type:'ad', active:0, all:156, d30:4},
  {id:'haven', name:'Haven', type:'ad', active:0, all:134, d30:1},
  {id:'allen-morris-company', name:'Allen Morris Company', type:'partner', active:0, all:119, d30:5},
  {id:'quinn-pr', name:'Quinn PR', type:'partner', active:0, all:119, d30:5},
  {id:'c-r-pr', name:'C&R PR', type:'partner', active:0, all:114, d30:6},
  {id:'related-ross', name:'Related Ross', type:'partner', active:0, all:108, d30:6},
  {id:'carli-pr', name:'Carli PR', type:'partner', active:0, all:91, d30:6},
  {id:'serhant', name:'SERHANT.', type:'partner', active:0, all:88, d30:6},
  {id:'little-palm-island', name:'Little Palm Island', type:'ad', active:0, all:85, d30:7},
  {id:'nora-banner', name:'Nora Banner', type:'ad', active:0, all:80, d30:0},
  {id:'one-aldwych', name:'One Aldwych', type:'ad', active:0, all:76, d30:4},
  {id:'novel', name:'NOVEL', type:'ad', active:0, all:66, d30:0},
  {id:'duos-hotel-condominiums-in-the-heart-of-miami', name:'DUOS - Hotel Condominiums in the Heart of Miami', type:'ad', active:0, all:60, d30:3},
  {id:'the-replay-club', name:'The Replay Club', type:'ad', active:0, all:52, d30:3},
  {id:'palm-cove-golf-club', name:'Palm Cove Golf Club', type:'ad', active:0, all:46, d30:5},
  {id:'streamsong-resort', name:'Streamsong Resort', type:'ad', active:0, all:45, d30:3},
  {id:'delano-residences-miami', name:'Delano Residences Miami', type:'ad', active:0, all:30, d30:0},
  {id:'aqua', name:'AQUA', type:'ad', active:0, all:21, d30:0},
  {id:'the-ritz-carlton-residences-sarasota-bay', name:'The Ritz-Carlton Residences Sarasota Bay', type:'ad', active:0, all:13, d30:0},
];
let _advReady = false;
async function ensureAdvertisersTable(env) {
  ...
  try {
    // Upsert the seed: refresh name/type + the (bot-filtered) Linkly baseline on
    // every deploy, but PRESERVE `active` on conflict so a dashboard live/off
    // toggle is never clobbered (active is only set on first insert).
    const now = Math.floor(Date.now() / 1000);
    const stmt = env.DB.prepare(
      'INSERT INTO advertisers (id, name, type, active, linkly_all, linkly_30d, updated_at) ' +
      'VALUES (?1,?2,?3,?4,?5,?6,?7) ' +
      'ON CONFLICT(id) DO UPDATE SET name=?2, type=?3, linkly_all=?5, linkly_30d=?6, updated_at=?7'
    );
...
async function _factVerifyCall(env, claims, facts, extra) {
  const sys = 'You are the fact-checker for a real-estate development publication. For each numbered claim: verify against the SOURCE NOTES first — the TMW DATABASE section, when present, is AUTHORITATIVE for the projects it lists (a matching claim is "verified" with source "TMW database"; a conflicting one is "contradicted" with the database value as the correction — no web search needed either way). Only when the notes do not settle a claim, SEARCH THE WEB — do NOT mark a claim "unsupported" without actually searching for it (well-known facts about places, brands, and designers are usually one search away). Be strict about tense: a property that has not opened yet must NEVER be described as open, opened, or debuted. Output ONLY a JSON array, one entry per claim (and one for ALSO CHECK if present, using i=-1): [{"i":<claim number>,"verdict":"verified"|"unsupported"|"contradicted","source":"<url, \'provided facts\', or \'TMW database\'>","note":"<if contradicted or unsupported: the correct/current fact in one short sentence>"}]. "unsupported" = searched and still could not confirm.';
  const usr = (facts ? 'SOURCE NOTES:\n' + String(facts).slice(0, 9000) + '\n\n' : '')
...
.card{position:relative;z-index:1;overflow:hidden;width:100%;max-width:420px;min-width:0;margin:auto;text-align:center;background:rgba(9,9,11,.92);
...
//   offer   — approved media in-kind offers not yet experienced; location =
//             the specific place (e.g. "Colorado" under Rockies of Tomorrow)
async function ensureFlowsTable(env) {
  ...
  for (const col of ['invoice_date TEXT', 'received_date TEXT', 'location TEXT']) {
...
function flowClean(b) {
  ...
  const kind = ['income', 'inkind', 'pending', 'plan', 'offer'].includes(b.kind) ? b.kind : null;
  let expenses = null;
...
async function handleAdminFlowSave(request, env, origin) {
  ...
  try {
    ...
    if (id) {
      ...
      await env.DB.prepare(`UPDATE flows SET kind=?,year=?,date=?,amount=?,description=?,party=?,paid_by=?,category=?,status=?,type=?,star=?,notes=?,expenses_json=?,plan_day=?,invoice_date=?,received_date=?,location=?,updated_at=? WHERE id=?`)
        .bind(f.kind, f.year, f.date, f.amount, f.description, f.party, f.paid_by, f.category, f.status, f.type, f.star, f.notes, f.expenses_json, f.plan_day, f.invoice_date, f.received_date, f.location, now, id).run();
    } else {
      id = 'fl-' + crypto.randomUUID();
      await env.DB.prepare(`INSERT INTO flows (id,kind,year,date,amount,description,party,paid_by,category,status,type,star,notes,expenses_json,plan_day,invoice_date,received_date,location,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
        .bind(id, f.kind, f.year, f.date, f.amount, f.description, f.party, f.paid_by, f.category, f.status, f.type, f.star, f.notes, f.expenses_json, f.plan_day, f.invoice_date, f.received_date, f.location, now, now).run();
    }
...
async function qboTokenRequest(env, params) {
  ...
  if (!r.ok) throw new Error('token exchange ' + r.status + ' [tid ' + (r.headers.get('intuit_tid') || '-') + ']: ' + (await r.text()).slice(0, 300));
  return r.json();
}
...
async function qboQueryAll(env, auth, entity, where) {
  ...
  for (let start = 1; start < 5000; start += 1000) {
    ...
    if (!r.ok) throw new Error(entity + ' query ' + r.status + ' [tid ' + (r.headers.get('intuit_tid') || '-') + ']: ' + (await r.text()).slice(0, 300));
    const d = await r.json();
