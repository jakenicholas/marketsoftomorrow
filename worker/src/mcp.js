...
const IMPL = {
  ...
  async generate_article_draft(args, env) {
    ...
    const sys = [
      ...
      'RULES: Write in TMW\'s voice per the brand brain above — hooky, confident, concrete, forward-looking. Do NOT invent facts, numbers, dates, prices, unit counts, or firm names beyond the facts provided and what is genuinely, verifiably known. NEVER fabricate a quotation or attribute words to any person, team, or company: include quoted speech ONLY if it appears verbatim in the provided facts. Strong hook, scannable structure, no corporate/press-release tone. Do not embed images (they are inserted separately). LINKS: only add external links for source attribution (publications, official announcements); NEVER link a project, firm, or city name to an external site — mentions of tracked projects, firms, and markets are auto-linked to their oftmw.com pages after generation. HOW IT ENDS: land on a real closing PARAGRAPH, forward-looking, in your own prose. NEVER end with a call to action, a newsletter or subscribe pitch, a follow-us line, or any sign-off boilerplate (our older archive is full of "Sign up for ... free newsletter below" endings; that branding is retired and they must never be reproduced). NEVER end on an image either: photos belong between paragraphs, and the last thing a reader sees is your conclusion.',
  ...
  async revise_article_draft(args, env) {
    ...
    const sys = [
      ...
      'RULES: Preserve every fact from the original (do NOT invent or drop verified facts, numbers, dates, prices, or firm names). NEVER fabricate a quotation or attribute words to anyone. TMW voice per the brand brain. Return the whole article, not a diff.',
  ...
  async propose_project_edit(args, env) {
    ...
    const ALLOWED = new Set(['name', 'status', 'city', 'borough', 'neighborhood', 'street', 'postal_code', 'country', 'lat', 'lng', 'official_website',
    ...
    const fromVal = (k) => {
      if (!live) return null;
      const m = {
        name: live.Title, status: live.Status || live.Delivery || '', city: live.City, borough: live.Borough,
