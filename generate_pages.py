...
<body>
  ...
  <script>
    ...
    (function watchBtnHydrate() {{
      ...
      function renderWatchingCard(count, lastEvent) {{
      ...
      waitForMemberstack(async function(ms) {{
        ...
        if (isWatching) {{
          ...
          fetchUnread(lastViewed[slug] || '', function(count, lastEvent) {{
            renderWatchingCard(count, lastEvent);
            // Stamp last_viewed_at = now AFTER reading the previous timestamp
            // so the counter accurately reflects pre-view state.
