    ...
    (function () {{
      function fitFirms() {{
        ...
        for (var g = 0; g < grids.length; g++) {{
          ...
          var nms = grid.querySelectorAll('.pp-firm-chip .nm');
          var clipped = false;
          for (var i = 0; i < nms.length; i++) {{ if (nms[i].scrollWidth > nms[i].clientWidth + 1) {{ clipped = true; break; }} }}
          if (clipped) grid.classList.add('pp-firms-stack');
        }}
      }}
