    ...
    return f'''
      <div class="pp-sec ai-sec" id="atlasIntel" data-slug="{html.escape(slug)}" data-supply="{supply_level}">
        <style>
          ...
          .ai-crow{{display:flex;align-items:center;gap:12px;padding:8px 0;font-size:13px}}
          .ai-crow .cn{{flex:1;color:#f4f4f5;font-weight:500;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}
          .ai-crow .cm{{color:var(--ai-muted);font-size:12px;white-space:nowrap}}
          .ai-crow .cb{{width:56px;height:4px;border-radius:2px;background:var(--ai-stroke2);position:relative;overflow:hidden;flex:0 0 auto}}
          .ai-crow .cb i{{position:absolute;left:0;top:0;bottom:0;background:var(--ai-green);opacity:.6;border-radius:2px}}
          .ai-crow .cp{{font-family:var(--ai-mono);font-variant-numeric:tabular-nums;font-weight:500;white-space:nowrap}}
        ...
        (function(){{
          ...
          function paint(m){{
            ...
            var rows = (m.comps || []).map(function(c){{
              ...
              return '<div class="ai-crow"><span class="cn">' + c.title + '</span><span class="cm">' + c.miles + ' mi · ' + c.meta + '</span><div class="cb"><i style="width:' + Math.round(c.psf_modeled / maxP * 100) + '%"></i></div><span class="cp">' + money(c.psf_modeled) + '</span></div>';
            }}).join('');
