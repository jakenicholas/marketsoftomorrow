...
.rel-card{display:grid;grid-template-columns:200px 1fr;gap:22px;border:1px solid var(--hair);border-radius:18px;overflow:hidden;background:linear-gradient(180deg,rgba(167,139,250,.035),transparent 60%);transition:border-color .2s,transform .2s,box-shadow .2s}
.rel-card:hover{border-color:rgba(167,139,250,.42);transform:translateY(-2px);box-shadow:0 24px 60px -30px rgba(167,139,250,.5)}
.rel-card .rc-media{position:relative;overflow:hidden;background:#111}
...
.rel-card .rc-body{padding:22px 24px 22px 0;display:flex;flex-direction:column;justify-content:center;min-width:0}
...
.rel-card h2{font-family:var(--serif);font-size:clamp(22px,2.6vw,28px);font-weight:500;letter-spacing:-.014em;line-height:1.12;color:var(--white);margin:10px 0 8px}
.rel-card .rc-dek{font-size:14px;color:var(--mute2);line-height:1.55}
...
@media(max-width:620px){.rel-card{grid-template-columns:1fr}.rel-card .rc-media{aspect-ratio:16/9}.rel-card .rc-body{padding:0 22px 24px}}
...
def build_reader(ed):
    ...
    body = f"""
<div class="wrap">
  <nav class="rel-crumbs"><a href="/">TMW</a><span class="sep">/</span><a href="/releases/">Releases</a><span class="sep">/</span>Edition {ed['num']}</nav>
...
def build_index():
    ...
    body = f"""
<div class="wrap">
  <nav class="rel-crumbs"><a href="/">TMW</a><span class="sep">/</span>Releases</nav>
  <header class="rel-hero">
    <span class="rel-eyebrow">The Weekly</span>
    <h1>The build log.</h1>
    <p class="sub">Every edition of The Weekly — what we shipped, and why it compounds. Newest on top.</p>
  </header>
